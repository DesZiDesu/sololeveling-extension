...
const TABS = [
    ...
    { id: 'summons', labelKey: 'tab_summons', icon: 'fa-solid fa-users-rays', conditional: true },
...
function normalizeSkill(source = {}) {
    ...
    return {
        ...
        buff: {
            ...
            auraColor: hexColor(buffSource.auraColor || source.auraColor, shadowExtraction || /shadow\s*domain/i.test(name) ? '#8b5cf6' : '#35bfff'),
            backgroundColor: hexColor(buffSource.backgroundColor, shadowExtraction || /shadow\s*domain/i.test(name) ? '#10081f' : '#030e1c'),
            particleColor: hexColor(buffSource.particleColor, shadowExtraction || /shadow\s*domain/i.test(name) ? '#c4a7ff' : '#6dd8ff'),
...
function normalizeShadow(source = {}, skills = []) {
    ...
    return {
        ...
        manaCost: number(source.manaCost, 0, 0, 999999),
...
function summonUnitCard(unit) {
    ...
    return `<article class="sl-summon-card"><button type="button" class="sl-summon-card-main" data-sl-shadow="${html(unit.id)}"><span class="sl-summon-avatar"><i class="fa-solid ${unit.kind === 'Shadow' ? 'fa-user-ninja' : 'fa-paw'}"></i><em>${html(unit.rank)}</em></span><span class="sl-summon-identity"><small>${html(unit.kind)} · ${html(unit.class)}</small><b>${html(unit.name)}</b><em>LV. ${unit.level} · ${html(unit.status)}</em></span><i class="fa-solid fa-chevron-right"></i></button><button type="button" class="sl-summon-quick" data-sl-summon-action="${deployed ? 'store' : 'deploy'}" data-summon-id="${html(unit.id)}"><i class="fa-solid ${deployed ? 'fa-arrow-rotate-left' : 'fa-burst'}"></i>${deployed ? t('recall') : t('deploy')}</button></article>`;
}
...
function renderSummons() {
    ...
    return `<section class="sl-module-heading"><div><span class="sl-system-eyebrow">${t('summon_authority')}</span><h3>${t('shadow_storage')}</h3></div><strong>${living.length} / ${capacity}</strong></section><section class="sl-summon-overview"><div class="sl-capacity-ring" style="--capacity:${progress}"><span><b>${living.length}</b><small>/ ${capacity}</small></span></div><div class="sl-summon-overview-copy"><span>${html(primary?.name || 'Summoning Skill')}</span><h4>${capacity - living.length} open slot${capacity - living.length === 1 ? '' : 's'}</h4><p>Capacity grows with summoning skill levels. ${primary ? `Next ${html(primary.name)} level adds ${primary.summonCapacityPerLevel}.` : ''}</p></div><dl><div><dt>${t('deployed')}</dt><dd>${deployed.length}</dd></div><div><dt>${t('stored')}</dt><dd>${stored.length}</dd></div><div><dt>${t('dismissed')}</dt><dd>${dismissed.length}</dd></div></dl></section>${deployed.length ? `<section class="sl-summon-group"><header><h4>${t('deployed')}</h4><span>${deployed.length}</span></header><div class="sl-summon-grid">${deployed.map(summonUnitCard).join('')}</div></section>` : ''}<section class="sl-summon-group"><header><h4>${t('stored')}</h4><span>${stored.length}</span></header>${stored.length ? `<div class="sl-summon-grid">${stored.map(summonUnitCard).join('')}</div>` : `<div class="sl-summon-empty"><i class="fa-solid fa-circle-nodes"></i><p>${t('no_summons')}</p></div>`}</section>${dismissed.length ? `<details class="sl-dismissed-archive"><summary>${t('dismissed')} <span>${dismissed.length}</span></summary><div>${dismissed.map(unit => `<button type="button" data-sl-shadow="${html(unit.id)}"><span>${html(unit.name)}</span><small>${html(unit.rank)} · ${html(unit.class)}</small></button>`).join('')}</div></details>` : ''}`;
}
...
function showSkillModal(skillId) {
    ...
    modal.innerHTML = `<button class="sl-submodal-backdrop" type="button" data-sl-action="close-modal"></button><article class="sl-item-sheet sl-skill-sheet"><header><span>SKILL INFORMATION</span><button type="button" data-sl-action="close-modal"><i class="fa-solid fa-xmark"></i></button></header><div class="sl-item-sheet-hero">${imageFrame(skill.icon, 'Skill', 'is-large')}<div><span>${html(skill.type)} · ${html(skill.rank)}-RANK</span><h3>${html(skill.name)}</h3><p>Level ${skill.level} · Used ${skill.uses} time${skill.uses === 1 ? '' : 's'}${cooldown ? ` · Cooldown ${cooldown}s` : ''}</p></div></div><section><h4>DESCRIPTION</h4><p>${html(skill.description)}</p></section><section><h4>MASTERY</h4><div class="sl-skill-sheet-mastery"><div class="sl-quest-progress"><i style="width:${percent(skill.mastery, skill.masteryRequired)}%"></i></div><b>${skill.mastery} / ${skill.masteryRequired}</b></div></section>${skill.activationRequired ? `<section><h4>VOICE COMMANDS</h4><form id="sl-activation-form" class="sl-activation-form"><label><i class="fa-solid fa-microphone-lines"></i><textarea id="sl-activation-words" maxlength="800" rows="3" placeholder="Arise, Rise, Answer my call">${html(skill.activationWords.join(', '))}</textarea></label><button type="submit">SAVE COMMANDS</button></form><p class="sl-muted-copy">Add up to 10 phrases separated by commas or new lines. They are stored only for this skill and chat.</p></section>` : ''}${skill.buffing ? `<section class="sl-aura-editor"><h4>INTERFACE AURA</h4><form id="sl-aura-form"><div class="sl-aura-colors"><label><span>Aura</span><input id="sl-aura-color" type="color" value="${html(skill.buff.auraColor)}"></label><label><span>Surface</span><input id="sl-aura-background" type="color" value="${html(skill.buff.backgroundColor)}"></label><label><span>Particles</span><input id="sl-aura-particle" type="color" value="${html(skill.buff.particleColor)}"></label></div><div class="sl-aura-numbers"><label><span>Duration (seconds)</span><input id="sl-aura-duration" type="number" min="5" max="86400" value="${skill.buff.durationSeconds}"></label><label><span>Cooldown (seconds)</span><input id="sl-aura-cooldown" type="number" min="0" max="604800" value="${skill.buff.cooldownSeconds}"></label><label><span>MP per reply</span><input id="sl-aura-drain" type="number" min="0" max="999999" value="${skill.buff.mpDrain}"></label></div><button type="submit">SAVE AURA</button></form>${skill.buff.enabled ? `<button type="button" class="sl-aura-disable" data-sl-action="disable-aura"><i class="fa-solid fa-power-off"></i> DISABLE ACTIVE AURA</button>` : '<p class="sl-muted-copy">The aura activates when the story confirms this skill was used.</p>'}</section>` : ''}<footer><button type="button" data-sl-action="edit-selected-skill-image"><i class="fa-solid fa-image"></i> EDIT ICON</button><button type="button" data-sl-action="close-modal">CLOSE</button></footer></article>`;
    modal.hidden = false;
}
...
function showShadowModal(shadowId) {
    ...
    modal.innerHTML = `<button class="sl-submodal-backdrop" type="button" data-sl-action="close-modal"></button><article class="sl-item-sheet sl-shadow-sheet"><header><span>${html(shadow.kind.toUpperCase())} RECORD</span><button type="button" data-sl-action="close-modal"><i class="fa-solid fa-xmark"></i></button></header><div class="sl-shadow-hero"><span class="sl-shadow-modal-mark"><i class="fa-solid ${shadow.kind === 'Shadow' ? 'fa-user-ninja' : 'fa-paw'}"></i><em>${html(shadow.rank)}</em></span><div><span>${html(shadow.class)} · ${owner ? html(owner.name) : 'Unknown authority'}</span><h3>${html(shadow.name)}</h3><p>Level ${shadow.level} · ${html(shadow.status)}${shadow.manaCost ? ` · ${shadow.manaCost} MP` : ''}</p></div></div><section><h4>DESCRIPTION</h4><p>${html(shadow.description)}</p></section><section><h4>STATS</h4><div class="sl-shadow-stats">${Object.entries(shadow.stats).map(([key, value]) => `<article><span>${labels[key] || html(key)}</span><b>${value}</b></article>`).join('')}</div></section><section><h4>ABILITIES</h4><div class="sl-shadow-abilities">${shadow.abilities.map(ability => `<span>${html(ability)}</span>`).join('') || '<p class="sl-muted-copy">No abilities recorded.</p>'}</div></section>${!shadow.dismissed ? `<section class="sl-summon-management"><h4>MANAGEMENT</h4><div><button type="button" data-sl-summon-action="${shadow.status === 'Deployed' ? 'store' : 'deploy'}" data-summon-id="${html(shadow.id)}"><i class="fa-solid ${shadow.status === 'Deployed' ? 'fa-arrow-rotate-left' : 'fa-burst'}"></i>${shadow.status === 'Deployed' ? t('recall') : t('deploy')}</button><button type="button" class="sl-dismiss-soul${armed ? ' is-armed' : ''}" data-sl-summon-action="${armed ? 'confirm-dismiss' : 'arm-dismiss'}" data-summon-id="${html(shadow.id)}"><i class="fa-solid fa-link-slash"></i>${armed ? t('confirm_dismiss') : t('dismiss_soul')}</button></div>${armed ? '<p>This permanently releases the soul, frees one storage slot, and cannot be undone.</p>' : ''}</section>` : `<section class="sl-dismissed-record"><i class="fa-solid fa-feather-pointed"></i><p>This soul was permanently released and no longer uses capacity.</p></section>`}<footer><button type="button" data-sl-action="close-modal">CLOSE</button></footer></article>`;
    modal.hidden = false;
}
